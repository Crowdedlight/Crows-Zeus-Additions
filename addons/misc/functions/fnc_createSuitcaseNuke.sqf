...
private _onConfirm = {
    ...
    [{
        ...
        if(!isNil "_newCountdown") then {
            ...
            _device setVariable [QGVAR(suitcaseNuke_newCountdown), nil];
            _device setVariable [QGVAR(suitcaseNuke_countdown), _countdownRemaining];
        };
        ...
        if(alive _device and _isArmed) then {
            ...
            private "_time";
            if(isMultiplayer) then { _time = {serverTime}; } else { _time = {time}; };
            ...
            _device setVariable [QGVAR(suitcaseNuke_time), (call _time)];
        ...
        if(!alive _device || !_isArmed) then {
            _device setVariable [QGVAR(suitcaseNuke_countdownFlipFlop), (_flipFlop+1)%4];
        } else {
            // In case device is re-armed while flipFlop=3
            _device setVariable [QGVAR(suitcaseNuke_countdownFlipFlop), 0];
        };
